...
for k=1:Ntests
    ...
    if imex_type == 1
        [t, y11, ctime(k)] = IMEX_I_A(@fun_f1, t0,T,y0,N(k),alpha,lambdamrho, ...
            A,Mlu,sigma1,Mf,sigma2,Mu,sigma3, Me,sigma4,tol,epsil);

    elseif imex_type == 2
        [t, y11, ctime(k)] = IMEX_II_A(@fun_f1, t0,T,y0,N(k),alpha,lambdamrho, ...
            A,Mlu,sigma1,Mf,sigma2,Mu,sigma3, Me,sigma4,tol,epsil);
    end
