function [t, y, ctime]=IMEX_II_A(Rfun,t0,T,y0,N,alpha,lambda,Al,Mlu,sigma1,Mf,sigma2,Mu,sigma3,Me,sigma4,tol,epsil)
% IMEX_II Second-order IMEX solver for a system of nonlinear FDEs
%   IMEX_II solver a nonlinear FDE of the form:
%
%   {}^C_{0} D_t^\alpha u(t) = A u(t) + f(t,u(t)), \alpha \in (0,1)
%
% Inputs:
%
...
%  lambda: a constant
%  Al: A matrix of coefficients for the linear part on the right-hand-side
...
if Mlu>0 || Mf>0 || Mu>0 || Me>0
   ...
   Flu=zeros(N,Mlu);  
   for k=1:Mlu
       Flu(1,k)=gamma(sigma1(k)+1)/gamma(sigma1(k)+alpha+1) - 1/gamma(alpha+2);
       for j=2:N
           Flu(j,k)=(j-1)^sigma1(k)/gamma(alpha+1) * gjquadreal2f1(-sigma1(k),1,alpha+1,-1/(j-1),0,200) ...
               - alpha*(j-1)^sigma1(k)/gamma(alpha+2) - j^sigma1(k)/gamma(alpha+2);
       end
       rhs1(:,k)=Flu(:,k);
   end
   ...
   Force=zeros(N,Mf);  
   for k=1:Mf
       Force(1,k)=gamma(sigma2(k)+1)/gamma(sigma2(k)+alpha+1) - 1/gamma(alpha+2);
       for j=2:N
           Force(j,k)=(j-1)^sigma2(k)/gamma(alpha+1) * gjquadreal2f1(-sigma2(k),1,alpha+1,-1/(j-1),0,200) ...
               - alpha*(j-1)^sigma2(k)/gamma(alpha+2) - j^sigma2(k)/gamma(alpha+2);
       end
       rhs2(:,k)=Force(:,k);
   end
   Wf = V2\rhs2; % Getting correction weights
   
   %%%% Computing the weights for the history 
   V3 = zeros(Mu);
   for i=1:Mu
       for k=1:Mu
           V3(i,k) = i^sigma3(k);
   ...
   for k=1:Mu
       ...
       raux = [r(3:N)'; 0; zeros(N-3,1)]; % Circulant embedding
       jaux = [((2:N-1).^sigma3(k))'; zeros(N-2,1)];    
       res = [0; real(ifft( fft(raux).*fft(jaux) ))];
      
       for j=2:N

           FF3=(r1(j)+res(j-1,1))/(gamma(alpha)*gamma(2-alpha));

           History(j,k)=- gamma(1+sigma3(k))*j^(alpha-1) ...
               * ((sigma3(k)-alpha+1)*(j)^(sigma3(k)-alpha+1)*betainc((j-1)/j,sigma3(k)-alpha+1,alpha)*auxbeta)/(gamma(alpha)*gamma(sigma3(k)-alpha+2)) ...
               + (j-1)^sigma3(k) - FF3;
       end
       rhs3(:,k)=History(:,k);
   end
   ...
   A1=zeros(N,1);
   A1(1)=1;
   A1(2)=-1+( (a(2,2)-3*a(2,3))/2 + (b(2,1)-b(2,2))/(2-alpha) )/(gamma(alpha)*gamma(2-alpha));
   A1(3)=( (a(3,2)-3*a(3,3)+4*a(3,4))/2 + (b(3,1)-3*b(3,2)+2*b(3,3))/(2-alpha) )/(gamma(alpha)*gamma(2-alpha));
   for i=4:N
       A1(i)=( (a(i,2)-3*a(i,3)+3*a(i,4)-a(i,5))/2 + (b(i,1)-3*b(i,2)+3*b(i,3)-b(i,4))/(2-alpha) )/(gamma(alpha)*gamma(2-alpha));
   end
   
   A2=zeros(N,1);
   A2(1)=1;
   A2(2)=-1+1/(gamma(alpha)*gamma(2-alpha))*(a(1,1)-a(1,2));
   A2(3)=1/(gamma(alpha)*gamma(2-alpha))*(a(2,1)-a(2,2)+2*a(2,3) - (2*b(2,1) - 2*b(2,2))/(2-alpha) );
   for i=4:N
       A2(i)=1/(gamma(alpha)*gamma(2-alpha))*((2*a(i-1,1)-2*a(i-1,2)+3*a(i-1,3)-a(i-1,4))/2 + (-2*b(i-1,1)+3*b(i-1,2)-b(i-1,3))/(2-alpha));
   end
   ...
   C=zeros(N,d);
   C(1,:)=y0+alpha*h^alpha/gamma(alpha+2)*(lambda*y0 + y0*Al' + feval(Rfun,t0,y0,alpha,lambda));
   C(2,:)=1/(gamma(alpha)*gamma(2-alpha))*y0*(a(1,1)-a(1,2)) - h^alpha/gamma(alpha+2)*feval(Rfun,t0,y0,alpha,lambda);
   for i=3:N
       C(i,:)=1/(gamma(alpha)*gamma(2-alpha))*y0*(a(i-1,1)-a(i-1,2)) ...
           -1/(gamma(alpha)*gamma(2-alpha))*y0*(-(a(i-1,2)+a(i-1,3))/2 + (b(i-1,1)-b(i-1,2))/(2-alpha));
   end
   ...
   while (norm(e)>tol)
       ...
       FC(1:N,:)=lambda*h^alpha*Wlu(1:N,1:Mlu)*(Y(2:Mlu+1,:) - y0) ...
           + h^alpha*Wlu(1:N,1:Mlu)*(Y(2:Mlu+1,:) - y0)*Al' ...
           + h^alpha/gamma(alpha+2)*We(1:N,1:Me)*(F(2:Me+1,:)-F(1,:)) ...
       ...
       b2 = B1*Y(2:N+1,:)*Al' + B1*F(2:end,:) + B2*F(2:end,:) + C + FC + A2.*Y(2,:);
   ...
   while (norm(e)>tol)
       ...
       % initial conditions 
       b2 = B1*Y(2:N+1,:)*Al' +B1*F(2:end,:) + B2*F(2:end,:) + C + A2.*Y(2,:);
