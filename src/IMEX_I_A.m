function [t, y, ctime]=IMEX_I_A(Rfun,t0,T,y0,N,alpha,lambda,Al,Mlu,sigma1,Mf,sigma2,Mu,sigma3,Me,sigma4,tol,epsil)
...
if Mlu>0 || Mf>0 || Mu>0 || Me>0
   ...
   for k=1:Mlu
       ...
       for j=2:N
           Flu(j,k)=(j-1)^sigma1(k)/gamma(alpha+1) * gjquadreal2f1(-sigma1(k),1,alpha+1,-1/(j-1),0,200) - j^sigma1(k)/gamma(alpha+1);
       end
       rhs1(:,k)=Flu(:,k);
   end
   ...
   for k=1:Mf
       Force(1,k)=gamma(sigma2(k)+1)/gamma(sigma2(k)+alpha+1) - 1/auxgamma;
       for j=2:N
   ...
   for k=1:Mu
       ...
       raux = [r(2:N)'; 0; zeros(N-2,1)]; % Circulant embedding
       ...
       for j=2:N
           ...
           History(j,k)=- auxgamma*j^(alpha-1) ...
